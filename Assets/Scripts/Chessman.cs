...
public abstract class Chessman : MonoBehaviour
{
	...
	{
		return GetValidMoves(checkFriendlyKingSafety: false, board: newBoard).Any(move => move.isKill && (newBoard[move.y, move.x] & Cell.King) == Cell.King);
	}

	public bool CanKillCell(int y, int x)
	{
		return GetValidMoves(checkFriendlyKingSafety: false, board: Board.Instance.Cells).Any(move => move.y == y && move.x == x);
	}
